        ...
        <span class="inline-grid grid-cols-4 sm:grid-cols-4 xl:grid-cols-10 gap-3">
            ...
            if (count($json) != 0) {
                foreach ($json['results'] as $result) {
                    $movie_title = str_replace(' ', '-', $result['title']);
                    ?>
                    <span>
                        <a href="<?php echo $URL; ?>/movies?id=<?php echo $result['id']; ?>/<?php echo $movie_title; ?>">
...
<center>
    <div class="container mx-auto">
        <span class="inline-grid grid-cols-4 sm:grid-cols-4 xl:grid-cols-10 gap-3">
            ...
            if (count($json) != 0) {
                foreach ($json['results'] as $result) {
                    $tv_title = str_replace(' ', '-', $result['name']);
                    ?>
                    <span>
                        <a href="<?php echo $URL; ?>/tv?id=<?php echo $result['id']; ?>/<?php echo $tv_title; ?>&s=1&e=1">
                            <img class="w-40" src="https://image.tmdb.org/t/p/w185<?php echo $result['poster_path']; ?>" alt="Image">
                        </a>
                    </span>
                    <?php
